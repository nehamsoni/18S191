...
md"""
``B = `` $(@bind B_slider Slider(-2.5:.001:0.1; show_value=true, default=-1.3))
"""
...
let 
	𝑩= -2.5:.001:-0.1
	ees= ECS.(;B=𝑩,a=Model.a)
	plot(𝑩,ees,label="ECS vs B",legend=:topleft,ylabel="Change in temp [ECS]", xlabel="B [W/m²/K]")
end
...
# ╔═╡ 189dc550-2746-11eb-1779-71b0d3ec6189
mean(ECS_samples)

# ╔═╡ 1f148d9a-1fc8-11eb-158e-9d784e390b24
histogram(ECS_samples, size=(600, 250), label=nothing, xlabel="ECS temp [°C]", ylabel="samples")
...
# ╔═╡ c210e420-2746-11eb-3bec-35481b1d7b85
ECS_of_mean_B=ECS(B=-1.3,a=Model.a)

# ╔═╡ 971dab8a-2747-11eb-0245-add2ec8b3a3f
probability= count(x->x>ECS_of_mean_B,ECS_samples)/length(ECS_samples)

# ╔═╡ d34a1530-2747-11eb-1591-f7bfe1c368a7
md"""
...
but getting a value above $\text{ECS}(\overline{B})$ is as probable as getting below.
but accounting for the extream large values of ECS when B touches -> 0, made it somewhat neither better nor worse.
"""
...
simulated_rcp85_model = let
	...
	scatter!(p,[(ebm.t[end-20],ebm.T[end-20])],label="Global Temp at 2100",legend=:topleft)
...
prob_RCP26=count(x->x>16.0,temperature_response.(Model.CO2_RCP26,B_samples))/length(B_samples)

# ╔═╡ 44cef944-274f-11eb-3718-11b5485889a9
prob_RCP85=count(x->x>16.0,temperature_response.(Model.CO2_RCP85,B_samples))/length(B_samples)
...
@bind log_CO2 Slider(1:0.01:6,default=2)

# ╔═╡ 83edf94c-275c-11eb-20d1-179919eb8963
CO2=10^log_CO2
...
# ╔═╡ 3d6c9452-2762-11eb-39fd-13b733614d9d
