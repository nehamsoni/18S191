...
function collide_boundary(c::Coordinate, L::Number)
	Coordinate(clamp(c.x,-L,L),clamp(c.y,-L,L))
end
...
# ╔═╡ 51788e8e-0a31-11eb-027e-fd9b0dc716b5
	let
		long_trajectory = trajectory(Coordinate(0,0), 1000)
...
let 
	p = plot(ratio=1)
	for i ∈ 1:10
		plot_trajectory!(p,trajectory(Coordinate(),1000))
	end
	p
end

# ╔═╡ f08cd1e6-0b27-11eb-2335-236e7a44959e
trajectory(Coordinate(),1000,3)
...
# ╔═╡ bac60e60-0bb8-11eb-1dfa-796065fd226b
...
mutable struct Agent<: AbstractAgent
	...
	function Agent(c::Coordinate)
		new(c,S,0)
	end
end

# ╔═╡ db958af6-0b29-11eb-3e2a-a1918d287a3f
Agent(Coordinate(0,1))
...
function initialize(N::Number, L::Number)
	...
	p= [Agent(Coordinate(rand(-L:L),rand(-L:L))) for i in 1:N]
	p[rand(1:N)].status = I
	p
end
...
# ╔═╡ e8a8f3d0-0b34-11eb-2a88-c1cb8dc8e7e1
state(s::InfectionStatus) = if s == S
	"Susceptible"
elseif s == I
	"Infectious"
else
...
# ╔═╡ 2e3cae50-0b35-11eb-2a89-3922aae24ada
state(a::Agent) = state(a.status)
...
causes_outbreak = CollisionInfectionRecovery(1, 0.00001)

# ╔═╡ 269955e4-0a46-11eb-02cc-1946dc918bfa
does_not_cause_outbreak = CollisionInfectionRecovery(0.1, 0.000000001)
...
# ╔═╡ a95dfbe0-0bbf-11eb-32e2-f9d29f45eb75
state(a::SocialAgent) = state(a.status)
...
function visualize(agents::Vector, L; kwargs...)
	p=plot(ratio=1)
	plot!(p,make_tuple.(position.(agents));seriestype = :scatter,color=color.(agents),group= state.(agents),kwargs...)
	return p
end
...
let
	...
	tata = initialize(N, L)
	tata[1].status=R
	visualize(tata, L) # uncomment this line!
end
...
function interact!(agent::Agent, source::Agent, infection::CollisionInfectionRecovery)
	if position(agent) == position(source)
		...
		if agent.status==I && source.status==S && rand()<infection.p_infection
			source.status=I
...
function initialize_social(N, L)
	lis=LinRange(.1,.5,10)
	p=[SocialAgent(Coordinate(rand(-L:L),rand(-L:L)),S,0,rand(lis)) for i in 1:N]
	p[rand(1:N)].status=I
	return p
end
...
function interact!(agent::SocialAgent, source::SocialAgent, infection::CollisionInfectionRecovery)
	...
	# your code here
	blah=agent.social_score+source.social_score
	if position(agent) == position(source)
		if agent.status==S && source.status==I && blah>rand() && rand()<infection.p_infection
			agent.status=I
			source.num_infected+=1
			return nothing
		end
		if agent.status==I && source.status==S && blah>rand() && rand()<infection.p_infection
			source.status=I
			agent.num_infected+=1
			return nothing
		end
...
function step!(agents::Vector, L::Number, infection::AbstractInfection)
	n = rand(1:length(agents))
	agents[n].position=collide_boundary(agents[n].position + rand(possible_moves),L)
	for i in 1:length(agents)
		if i==n
			continue
		else
			interact!(agents[i],agents[n],infection)
		end
	end
	agents
end
...
let
	...
	for i in 1:k_sweep_max
		step!(agents,L,pandemic)
		push!(stot,count( a -> a.status==S  ,agents))
		push!(itot,count( a -> a.status==I  ,agents))
		push!(rtot, N-(stot[end]+itot[end]))
	end
	plot!(p,1:k_sweep_max,stot,label="Susceptible")
	plot!(p,1:k_sweep_max,itot,label="Infectious")
	plot!(p,1:k_sweep_max,rtot,label="Recovered")
end
...
let
    ...
    Tmax = 200
    kwargs=Dict(:xlim=>(-55,55), :ylim=>(-55,55), :size=>(600,300))
    @gif for t in 1:Tmax
        for i in 1:50N
            step!(x, L, pandemic)
        end
...
let
	...
	plot!(p,1:k_sweep_max,stot,label="Susceptible")
	plot!(p,1:k_sweep_max,itot,label="Infectious")
	plot!(p,1:k_sweep_max,rtot,label="Recovered",title="Causes")
	...
	plot!(t,1:k_sweep_max,stot,label="Susceptible")
	plot!(t,1:k_sweep_max,itot,label="Infectious")
	plot!(t,1:k_sweep_max,rtot,label="Recovered",title="Does Not")
...
let
	pandemic=CollisionInfectionRecovery(1, 0.0001)
	p=plot(legend=false)
	for sim in 1:50
		N=50
		L=40
		sweep=5000
		agents=initialize(N,L)
		Ss=Int[];Is=Int[];Rs=Int[];
		for i in 1:sweep
			step!(agents,L,pandemic)
			push!(Ss,count(a->a.status==S,agents))
			push!(Is,count(a->a.status==I,agents))
			push!(Rs,N-Ss[end]-Is[end])
		end
		plot!(p,1:sweep,Ss,color="blue",linealpha=.2)
		plot!(p,1:sweep,Is,color="red",linealpha=.2)
		plot!(p,1:sweep,Rs,color="green",linealpha=.2)
	end
	p
end
...
let
	...
	@gif for t in 1:Tmax
		...
		right = plot(xlim=(0,Tmax), ylim=(0,N), size=(600,300))
        plot!(right, 1:t, Ss, color=color(S), label="S")
        plot!(right, 1:t, Is, color=color(I), label="I")
        plot!(right, 1:t, Rs, color=color(R), label="R")
...
let 
	x=Float64[]
	y=Int[]
	for agen in social_agents
		push!(x,agen.social_score)
		push!(y,agen.num_infected)
	end
	t=scatter(x,y,title="who infected whom?",xlabel="social score",ylabel="num infected",legend=false)
	#vline!(t,[.2],color="purple")
end
...
# ╔═╡ e1a888e4-0bc9-11eb-3dde-61236a3e3160
let
	N=50
	L=40
	...
	@gif for t in 1:Tmax
		if t<lockdown
			...
			push!(Is,count(a->a.status==I,agents))
			push!(Rs,N-Ss[end]-Is[end])
			right = plot(xlim=(0,Tmax), ylim=(0,N), size=(1000,600))
        	plot!(right, 1:t, Ss, color=color(S), label="S")
        	plot!(right, 1:t, Is, color=color(I), label="I")
        	plot!(right, 1:t, Rs, color=color(R), label="R")
			plot(right)
		elseif t==lockdown
			...
			global lockdown_agents=deepcopy(agents)
			global lockdown_Ss=deepcopy(Ss)
			global lockdown_Is=deepcopy(Is)
			global lockdown_Rs=deepcopy(Rs)
			for lola in lockdown_agents
				lola.social_score*=.25
			end
			for i in 1:50N
				step!(agents,L,pandemic)
			...
			push!(lockdown_Ss,count(a->a.status==S,lockdown_agents))
			push!(lockdown_Is,count(a->a.status==I,lockdown_agents))
			push!(lockdown_Rs,N-lockdown_Ss[end]-lockdown_Is[end])
			
			right = plot(xlim=(0,Tmax), ylim=(0,N), size=(1000,600))
        	plot!(right, 1:t, Ss, color=color(S), label="without lock S",linealpha=.3)
        	plot!(right, 1:t, Is, color=color(I), label="without lock I",linealpha=.3)
        	plot!(right, 1:t, Rs, color=color(R), label="without lock R",linealpha=.3)
			
			
			plot!(right, 1:t, lockdown_Ss, color=color(S), label="locked S",linealpha=.7)
        	plot!(right, 1:t, lockdown_Is, color=color(I), label="locked I",linealpha=.7)
        	plot!(right, 1:t, lockdown_Rs, color=color(R), label="locked R",linealpha=.7)
			plot(right)
		else
			for i in 1:50N
				step!(agents,L,pandemic)
        	...
        	plot!(right, 1:t, lockdown_Is, color=color(I), label="locked I",linealpha=.7)
        	plot!(right, 1:t, lockdown_Rs, color=color(R), label="locked R",linealpha=.7)
			vline!(right,[lockdown],label="lockdown exposed", color="purple")
			plot(right)
		end
	end
end
