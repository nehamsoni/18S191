...
# ╔═╡ 4e8cf0b4-2fb8-11eb-2303-39e7852223db
using LinearAlgebra
...
begin
	U_ex_Slider = @bind U_ex Slider(-5:1:8, default=0, show_value=false);
	md"""
	$(U_ex_Slider)
	"""
end
...
Multiplying `Δt` by 2^8 we see a strange grid of checks (white & yellow) with mean temp falling 
bellow 0 °C pretty soon.
...
let
	T=1:200
	plot(T,runtime.(T);xlabel="N ",ylabel="Time (sec)",title="Complexity in terms of N",legend=false,color=:red)
end
...
Indeed, it turns out if we change α function to always return 0 we will always get a single stable point even for different values of S_mean. 

Hence albedo feedback effect bifuricates  ``T_{\text{equilibrium}}``
"""
...
$\begin{split}
\frac{\partial T}{\partial t} =&\ \hphantom{+} T_{x,y}(t) + S_{mean}

\end{split}$
where 
$\begin{split} T(t) \end{split}$ is 5 degree polynomial function of $\begin{split} t \end{split}$.

Hence, It more likely depends upon $\begin{split} S_{mean} \end{split}$ for $\begin{split} T_{x,y}(t) \end{split}$ to have how many roots with negitive slope (stable points). Thereby, $\begin{split} S_{mean} \end{split}$ predominantly dictates the number of Equilibrium Temprature points.
...
# ╔═╡ 66403206-2fdf-11eb-13e0-ddd628f37fea
begin
	S_val= [1000, 1250, 1380, 1550, 1650, 1750, 2000];
	T_val= [-50, 0, 50];
end

# ╔═╡ 59da0470-2b8f-11eb-098c-993effcedecf
# you can change me!
bifurcation_ST = [(S,T) for S in S_val for T in T_val]

# ╔═╡ 9eacf3d0-2e9d-11eb-3c19-1dcfadccf18d
Results = ThreadsX.map(bifurcation_ST) do P
	eq_T(P[1],P[2])
end

# ╔═╡ 2446b816-2fdc-11eb-3da7-2db8d0766bd8
let
	p= plot(S_val,transpose(reshape(Results,:,length(S_val)));label=["-50°C" "0°C" "50°C"],legend=:topleft,color=["yellowgreen" "deepskyblue2" "orange"],lw=2,alpha=0.5) 
	scatter!(p,map(x->x[1],bifurcation_ST),Results;xlabel="S Mean",ylabel="T equilibrium",color=:red,title="Bifurication of T equi Vs Smean",label="Equilibrium Temp",alpha=0.7)
end
