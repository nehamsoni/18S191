...
function euler_integrate(fprime::Function, fa::Number, 
	...
	h = step(T)
	res=Vector{Float64}()
	push!(res,euler_integrate_step(fprime,fa,a0,h))
	for i in T[2:end]
		t=euler_integrate_step(fprime,res[end],i,h)
		push!(res,t)
	end
...
function euler_SIR(β, γ, sir_0::Vector, T::AbstractRange)
	...
	#num_steps = length(T)
	push!(res,euler_SIR_step(β,γ,sir_0,h))
	for i ∈ T[2:end]
		t = euler_SIR_step(β,γ,res[end],h)
		push!(res,t)
	end
	return res
end
...
# ╔═╡ 567da4c4-16a1-11eb-06d7-cdbde5e4a21f
md"""β (Beta)"""

# ╔═╡ 68274534-1103-11eb-0d62-f1acb57721bc
@bind β Slider(0.0:0.01:1.0,show_value=true,default=0.3)

# ╔═╡ 0cc7dbbe-16a2-11eb-382c-c746b9ff6774
md"""γ (Gamma)"""

# ╔═╡ 79fa0b5a-16a0-11eb-0150-2536ea19c5ee
@bind γ Slider(0.0:0.01:1.0,show_value=true,default=0.15)

# ╔═╡ 253a39c8-16a2-11eb-3b75-eb3625688a9c
plot_sir!(plot(), sir_T, euler_SIR(β, γ, 
	[0.99, 0.01, 0.00], 
	sir_T))
...
function ∂x(f::Function, a, b)
	h=1e-10
	return finite_difference_slope(x->f(x,b),a,h)
end
...
function ∂y(f::Function, a, b)
	h=1e-10
	return finite_difference_slope(y->f(a,y),b,h)
end
...
function gradient_descent_1d_step(f, x0; η=0.01)
	...
	return x0-η*finite_difference_slope(f,x0,1e-6)
end
...
@bind N_gradient_1d Slider(0:200)
...
function gradient_descent_1d(f, x0; η=0.01, N_steps=1000)
	t = gradient_descent_1d_step(f,x0;η=η)
	for i in 1:N_steps
		t = gradient_descent_1d_step(f,t;η=η)
	end
	return t
end
...
let
	...
	gradient_descent_1d(f, 0.0;η=0.01,N_steps=100)
end
...
function gradient_descent_2d(f, x0, y0; η=0.01)
	...
	popo=η
	last=[x0,y0]
	new= gradient_descent_2d_step(f, x0, y0; η=popo)
	while true
		if f(last[1],last[2])-f(new[1],new[2])<error
			break
		else
			temp=new
			new= gradient_descent_2d_step(f, new[1], new[2]; η=popo)
			last=temp
		end
	end
	return last
...
function loss_dice(μ, σ)
	...
	res=map(gauss.(dice_x,μ,σ).-dice_y) do x
		x^2
	end
	sum(res)
end
...
found_μ, found_σ = let
	...
	p=gradient_descent_2d(loss_dice,30,1;η=100)
	p[1], p[2]
end
...
function loss_sir(β, γ)
	cal=euler_SIR(β,γ,[.99,.01,0.0],hw4_T)
	ans=0.0
	for i in hw4_T
		#ans+=(sum(map(abs,cal[i].-hw4_results[i]))/3)^2
		
		po=map(cal[i].-hw4_results[i]) do x
			x^2
		end
		ans+=sqrt(sum(po))
	end
...
# ╔═╡ 666c416a-16d3-11eb-3124-ffe26d331198
found_β, found_γ = let
	
	# your code here
	p=gradient_descent_2d(loss_sir,0.05,0.005;η=.000000001)
	p[1], p[2]
end
