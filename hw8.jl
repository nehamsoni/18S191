...
# ╔═╡ f89b5c3e-237a-11eb-2a79-ff61d1c515b7
RGBA(1,0,0,0)*.5

# ╔═╡ 5cda29ae-2386-11eb-1621-2bbc0e314f64
pallla= true ? 1 : 0

# ╔═╡ 5009a53e-2376-11eb-1a56-ade7afaf318e
begin
	function interact(ray::Photon, hit::Intersection{Sphere}, num::Int64, obj::Vector{Object})
		colour_ray=RGBA(0,0,0,0)
		if num==1
			colour_ray = hit.object.s.c
			return Photon(hit.point, ray.l, colour_ray, ray.ior)
		else
			ref=hit.object.s.r
			tra=hit.object.s.t
			view=1-ref-tra
			if ref!=0
				...
				pall=reflect(ray.l,sphere_normal_at(hit.point, hit.object))
				ref_ray=step_ray(Photon(hit.point,pall,ray.c,ray.ior),obj,num-1)
				colour_ray+= ref*ref_ray.c
			end
			
			if tra!=0
				
				old=ray.ior
				new=old==hit.object.s.ior ? 1.0 : hit.object.s.ior
				popo=refract(ray.l,sphere_normal_at(hit.point, hit.object),old,new)
				tra_ray=step_ray(Photon(hit.point,popo,ray.c,new),obj,num-1)
				colour_ray+= tra*tra_ray.c
			end
			
			if view!=0
				colour_ray+= view*hit.object.s.c
			end
			return Photon(hit.point, ray.l, colour_ray, ray.ior)
		end
	end
...
let
	scene = [sky]
	ray_trace(scene, basic_camera; num_intersections=1)
end
...
main_scene = [
	...
	Sphere([0,50,-100], 20, 
		Surface(0.4, 0.3, RGBA(0,1,0,0.0), 1.0)),
...
let
	cam = Camera((400,300), 16, -15, [0,10,100])

	ray_trace(main_scene, cam; num_intersections=1)
end
...
escher_cam = Camera((600,600), 30, -7, [0,00,30])
...
# ╔═╡ 78a22544-2389-11eb-104e-e3160b341348
fons_face=load(download("https://user-images.githubusercontent.com/6933510/98426463-31a64f00-2099-11eb-96dd-9f0f065e0da4.png"))

# ╔═╡ dcb06778-2391-11eb-0d03-c5a79d6874b0
fons_face2=fons_face[70:230,130:470]

# ╔═╡ c0e1a08a-2389-11eb-2350-f91207749077
let
	face_skybox = image_skybox(Gray.(fons_face2))
...
function padded(img)
	m,n=size(img)
	return padarray(img,Fill(0,(40,n),(40,n)))
end
