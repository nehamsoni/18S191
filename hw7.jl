...
# ╔═╡ af84fbb8-1da1-11eb-3513-41bfa2cc4d98
normalize([-3,1])
...
test_photon = Photon([-1, 2], normalize([50,50]), 1.0)
...
reflect(test_photon.l,test_wall.normal)
...
# ╔═╡ f0403290-1db1-11eb-25c7-576652272e93
repeat([[5]],2)
...
function intersection(photon::Photon, sphere::Sphere; ϵ=1e-3)
	...
	if (t1>ϵ && t2<ϵ) || (t1<ϵ && t2>ϵ)
		t=max(t1,t2)
		return Intersection(sphere, norm(t*photon.l), photon.p .+ t*photon.l)
	end
	if (t1>ϵ && t2>ϵ)
		t=min(t1,t2)
		return Intersection(sphere, norm(t*photon.l), photon.p .+ t*photon.l)
...
test_sphere = Sphere(
	philip.p .+ -7*[-philip.l[2],philip.l[1]],
	7,
	1.5,
)
...
function interact(photon::Photon, hit::Intersection{Sphere})
	...
	return Photon(hit.point,refract(photon.l,sphere_normal_at(hit.point,hit.object),old_ior,new_ior),new_ior)
end
...
function trace(photon::Photon, scene::Vector{<:Object}, N)
	accumulate(step_ray,repeat([scene],N);init=photon)
end
...
let
	...
	line = [test_lens_photon.p, [r.p for r in path]...]
	plot!(p, first.(line), last.(line), lw=3, color=:red)
...
# ╔═╡ 21520190-1dc3-11eb-2269-df21b8d540d9
md""" IOR of lens:- $(@bind sphere_ior Slider(1:0.1:5; default=1.5,show_value=true))"""

# ╔═╡ b93a3d62-1dc3-11eb-1d3b-750205836676
md""" steps $(@bind steping Slider(1:30; default=4))"""

# ╔═╡ e3d18f84-1dc2-11eb-13ab-ef82f189c55c
my_lens=Sphere([-3,0],6.8,sphere_ior)

# ╔═╡ a784e024-1dc4-11eb-2143-157424aa1776
allphotons=[Photon([-9.9,y],normalize([1,0]),1.0) for y in -7:7]

# ╔═╡ 270762e4-1ca4-11eb-2fb4-392e5c3b3e04
let
	scene = [my_lens, box_scene...]
	p = plot_scene(scene, legend=false, xlim=(-11,11), ylim=(-11,11))
	all_paths=[]
	for ray in allphotons
		path = accumulate(1:steping; init=ray) do old_photon, i
			step_ray(old_photon, scene)
		end
		push!(all_paths,path)
	end
	for (index,parth) in enumerate(all_paths)
		line = [allphotons[index].p, [r.p for r in parth]...]
