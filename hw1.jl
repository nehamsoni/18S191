...
student = (name = "Jazzy Doe", kerberos_id = "neham")
...
function vecvec_to_matrix(vecvec)
	a=reshape(vecvec[1],1,:)
	n=length(vecvec)
	for i = 2:n
		a=[a ;reshape(vecvec[i],1,:)]
	end
...
function mean_colors(image)
	...
	return (redness/num,green/num,blue/num) 
...
begin
	function myclamp(x::Number,hi=1,lo=0)
		if lo<=x && x<=hi
			return x
		end
		if x>hi
	...
	function noisify(x::Number, s)
		...
		return myclamp((2*s*rand()-s)+x)
	end
	
	function noisify(color::AbstractRGB, s)
		# you will write me in a later exercise!
		a=(2*s*rand()-s)
		return RGB(color.r+a,color.g+a,color.b+a)
	end
...
The image is unrecognisable with intensity 1.5 and above in the spectrum. Si
"""
...
function convolve_image(M::AbstractMatrix, K::AbstractMatrix)
	...
	for (i,j) in Iterators.product(1:m,1:n)
		P[i,j]=sum([extend_mat(M, x, y) for (x,y) in Iterators.product(i-l1:i+l1,j-l2:j+l2)].*K)
...
function with_gaussian_blur(image)
	m,n=size(image)
	red=zeros(Float64,m,n)
	green=zeros(Float64,m,n)
	blue=zeros(Float64,m,n)
	kernal=zeros(Float64,11,11)
	for (i,j) in Iterators.product(-5,5,-5:5)
		kernal[i+6,j+6]=exp(-1*((i^2)+(j^2))/2)
	end
	kernal=kernal./(sqrt(2*pi))
	kernal=kernal./(sum(kernal))
	
	for (x,y) in Iterators.product(1:m,1:n)
		red[x,y]=image[x,y].r
		green[x,y]=image[x,y].g
		blue[x,y]=image[x,y].b
	end
	red=convolve_image(red,kernal)
	green=convolve_image(green,kernal)
	blue=convolve_image(blue,kernal)
...
# ╔═╡ 52a06686-fef8-11ea-0c3a-736ff93f800c
sum([1 2;3 4])
